...
7. **Sanitizes filenames for disk** (Windows only): characters illegal in Windows filenames (`\ / : * ? " < > |`) are replaced with underscores in the on-disk copy. The `fileName` stored in Pure and in all log outputs is always the fully decoded filename — URL-decoded, then any literal HTML-entity artifacts (e.g. `&#769;`) decoded and Unicode-normalized (NFKC) — so it matches the true DSpace original exactly, independent of whatever Windows-safe name was used on disk.
