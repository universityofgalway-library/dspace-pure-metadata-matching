...
9. **GETs** the Pure record after each successful PUT to retrieve the `fileId` and `fileName` assigned by Pure to the newly linked file. These are written to the `pdf_matched_records` CSV.
